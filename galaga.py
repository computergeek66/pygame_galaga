...
#Static assignments
FORMATIONS = ['0,0,0,Y,0,0,Y,0,0,0',
             '0,0,G,G,G,G,G,G,0,0',
             '0,R,R,R,R,R,R,R,R,0',
             'B,B,B,B,B,B,B,B,B,B',
             'B,B,B,B,B,B,B,B,B,B',
...
def main():
    ...
    for i in range(STARCOUNTMAX):
        ...
        star = Effect(star_drawable, random.randrange(DISPLAYWIDTH), random.randrange(DISPLAYHEIGHT), "star")
        effects.append(star)
    ...
    while True:
        ...
        for effect in effects:
            DISPLAYSURF.blit(effect.drawable.sprite, (effect.drawable.rect.x,effect.drawable.rect.y))
            ...
            if(bullet.drawable.rect.y < 0 or bullet.drawable.rect.y > DISPLAYHEIGHT):
                ...
                if bullet.direction == -1:
                    for enemy in enemies:
                        if bullet.drawable.rect.colliderect(enemy.drawable.rect) == True:
                            enemy.hit()
                            if enemy.health <= 0:
                                explosion = Effect(ENEMYEXPLOSIONDRAWBLE, enemy.drawable.rect.centerx, enemy.drawable.rect.centery, "explosion")
                ...
                if bullet.direction == 1:
                    if bullet.drawable.rect.colliderect(player.rect) == True:
                        explosion = Effect(PLAYEREXPLOSIONDRAWABLE, player.rect.centerx, player.rect.centery, "player_explosion")
                        effects.append(explosion)
                        destroy(player)
                        destroy(bullet)
                        lives -= 1
        ...
        if pause == "False":
            ...
            if keys[K_a] and move_ticker == 0:
                move_ticker = moveTickerMax
                player.rect.x -= playerSpeed
            if keys[K_d] and move_ticker == 0:
                move_ticker = moveTickerMax
                player.rect.x += playerSpeed
    ...
    for formation in FORMATIONS:
        ...
        for enemy in formation.split(','):
            ...
            if sprite != 0:
                ...
                enemyDrawable = Drawable(sprite, originX + (xcount*22), originY + (ycount*22))
                if xcount%2 == 0:
...
def create_GUI():
    ...
    scoreLabelDisplayRect.centerx =  DISPLAYWIDTH*.2
    ...
    highScoreLabelDisplayRect.centerx =  DISPLAYWIDTH*.7
